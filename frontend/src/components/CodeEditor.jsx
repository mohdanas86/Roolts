...
function CodeEditor({ isScribbleMode, scribbleTool = 'pen', scribbleColor = '#ff0000', rightPanelWidth, terminalHeight, terminalOpen }) {
    ...
    const { showOutput, error, isExecuting, isSplitMode, setSplitMode } = useExecutionStore();
    const activeFile = (files || []).find((f) => f.id === activeFileId);
    const { theme, format, features, experimental, scribblePenSize, scribbleEraserSize, backgroundImage, backgroundOpacity } = useSettingsStore();
