...
class CodexAPIClient {
    ...
    async sendMinimalRequest() {
        ...
        try {
            ...
            if (response.headers) {
                this.lastRateLimits = this.extractRateLimitsFromHeaders(response.headers);
            ...
            if (axios_1.default.isAxiosError(error)) {
                ...
                if (error.response) {
                    ...
                    this.lastRateLimits = this.extractRateLimitsFromHeaders(error.response.headers);
                }
                else {
    ...
    extractRateLimitsFromHeaders(headers) {
        ...
        const primaryUsedPercent = this.parseHeaderFloat(headers, 'x-codex-primary-used-percent');
        console.log('Primary used percent:', primaryUsedPercent);
        if (primaryUsedPercent !== null) {
            rateLimits.primary = {
                used_percent: primaryUsedPercent,
                window_minutes: this.parseHeaderInt(headers, 'x-codex-primary-window-minutes'),
                resets_in_seconds: this.parseHeaderInt(headers, 'x-codex-primary-reset-after-seconds')
            };
        }
        // Parse secondary window (weekly limit)
        const secondaryUsedPercent = this.parseHeaderFloat(headers, 'x-codex-secondary-used-percent');
        console.log('Secondary used percent:', secondaryUsedPercent);
        if (secondaryUsedPercent !== null) {
            rateLimits.secondary = {
                used_percent: secondaryUsedPercent,
                window_minutes: this.parseHeaderInt(headers, 'x-codex-secondary-window-minutes'),
                resets_in_seconds: this.parseHeaderInt(headers, 'x-codex-secondary-reset-after-seconds')
            };
        }
        return (rateLimits.primary || rateLimits.secondary) ? rateLimits : null;
    }
    parseHeaderFloat(headers, name) {
...
exports.CodexAPIClient = CodexAPIClient;
