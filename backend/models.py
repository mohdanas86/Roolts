...
class User(db.Model):
    ...
    hf_token = db.Column(db.String(500))
...
def init_db(app):
    ...
    with app.app_context():
        ...
        try:
            ...
            # SQLite doesn't support 'IF NOT EXISTS' for columns easily, but we can catch errors
            db.session.execute(text("ALTER TABLE users ADD COLUMN hf_token VARCHAR(500)"))
            db.session.commit()
